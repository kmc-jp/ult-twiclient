import {getApi} from '../common/oauth';
const remote = require('electron').remote;
const Menu = remote.Menu;
//const MenuItem = remote.MenuItem;
const clipboard = require('electron').clipboard;
const dialog = remote.dialog;
const path = require('path');
const Vue = require('vue');
Vue.config.debug = true;
require('twitter-text');
const localStorageName = "ulttwiclient";
const setting = JSON.parse(localStorage[localStorageName] || '{}');
console.log(setting.tokens);
let api = null;

function anchorHTML(text, href) {
  return `<a onClick="require('shell').openExternal('${href}')" class="url">'${text}'</a>`;
}

window.addEventListener('load',()=>{
  Vue.filter('expand_url', (text, entities)=>{
    entities.urls.forEach((url)=>{
      text = text.replace(url.url, anchorHTML(url.display_url, url.expanded_url));
    });
    if (entities.media) {
      entities.media.forEach((m)=>{
        text = text.replace(m.url, anchorHTML(m.display_url, m.expanded_url));
      });
    }
    return text;
  });
  Vue.filter('twemoji', (text)=>{
    return twemoji.parse(text);
  });
  Vue.filter('html_escape', (text)=>{
    return text.replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&#039;');
  });
  Vue.component('small-tweet', {
    template: '#small-tweet',
    props: {
      tweet: Object,
      replysender: Boolean
    },
    data() {
      return {vm};
    }
  });
  Vue.filter('text_content', (html)=>{
    const dom = document.createElement('div');
    dom.innerHTML = html;
    return dom.firstChild.textContent;
  });
  // context menu on status
  const contextMenuForTweet = Menu.buildFromTemplate([
    {label: "Reply to this tweet", click: ()=>{vm.sendReply(vm.selectedTweet);}},
    {label: "Favorite this tweet", click: ()=>{vm.favoriteTweet(vm.selectedTweet);}},
    {type: 'separator'},
    {label: "Open tweet with Browzer", click: ()=>{clipboard.writeText(vm.jumpToTwitterURL(vm.selectedTweet));}},
    {label: "Get tweet URL", click: ()=>{clipboard.writeText(vm.createTwitterURL(vm.selectedTweet));}},
    {label: "Get tweet JSON", click: ()=>{clipboard.writeText(JSON.stringify(vm.selectedTweet));}}
  ]);
  // context menu on image
  const contextMenuForImage = Menu.buildFromTemplate([
    {label: "Save image file", click: ()=>{vm.saveImage();}}
  ]);
  const vm = new Vue({
    el: "#container",
    data: {
      newTweet: {
        text: "",
        in_reply_to_status_id: ""
      },
      selectedTweet: {},
      sendReplyDestTweet: {},
      me: {},
      streaming: false,
      showingMediaType: '',
      image: {},
      video: {},
      tweets: [],
      notifications: [],
      maxTweetLength: 140,
      borderOfLongTweet: 20,
      nowTime: new Date()
    },
    computed: {
      calculateRemainChar() {
        return this.maxTweetLength - twttr.txt.getTweetLength(this.newTweet.text);
      },
      isLongTweet() {
        return this.borderOfLongTweet > this.calculateRemainChar;
      },
      isExceededTweet() {
        return this.calculateRemainChar < 0;
      }
    },
    methods: {
      addTweet(tweet) {
        const _tweet = this.body(tweet);
        if (tweet.user.screen_name == this.me.screen_name && tweet.retweeted_status) {
          _tweet.retweeted = true;
        }
        if (_tweet.in_reply_to_status_id_str) {
          api.get('statuses/show', {id: _tweet.in_reply_to_status_id_str},
            (error, in_reply_to_status)=>{
              if (!error){
                _tweet.in_reply_to_status = {};
                Object.assign(_tweet.in_reply_to_status, in_reply_to_status);
                this.insertToTweets(tweet);
              } else {
                console.error(error);
              }
            }
          );
        } else {
          this.insertToTweets(tweet);
        }
      },
      insertToTweets(tweet) {
        let i = 0;
        for(;i<this.tweets.length;i++){
          if(this.tweets[i].id<tweet.id){break;}
        }
        this.tweets.splice(i,0,tweet);
        const MAX_TWEETS = 300;
        if(this.tweets.length>MAX_TWEETS){
          this.tweets.splice(MAX_TWEETS, this.tweets.length - MAX_TWEETS);
        }
        this.modifyScroll();
      },
      modifyScroll() {
        const tweetsDOM = this.$els.tweets;
        const scrollTop = tweetsDOM.scrollTop;
        const topTweetDOM = tweetsDOM.firstElementChild;
        const eps = 2;
        if (!topTweetDOM || scrollTop < eps) {
          return;
        }
        tweetsDOM.scrollTop += topTweetDOM.clientHeight;
      },
      sendTweet(params) {
        api.post('statuses/update', {
          status: params.text,
          in_reply_to_status_id: params.in_reply_to_status_id
        }, (error, tweet, response)=>{
          if (error) {
            this.createNotification("Failed to send tweet", this.newTweet.text, null, 'fail');
            return console.log('error', error.map((e)=>e.message).join("\n"),  error);
          }
          console.log(tweet, response);
          this.createNotification("Sent tweet", this.newTweet.text, null, 'tweet');
          this.newTweet.text = '';
          this.newTweet.in_reply_to_status_id = '';
          this.sendReplyDestTweet = {};
        });
      },
      clearReply() {
        this.newTweet.text = this.newTweet.text.replace(/@[a-zA-Z0-9_]+\s*/g, '');
        this.newTweet.in_reply_to_status_id = '';
        this.sendReplyDestTweet = {};
      },
      startStreaming() {
        console.info("start streaming");
        this.streaming = true;
        api.stream('user', {}, (stream)=>{
          stream.on('data', (data)=>{
            console.log(data);
            if (data.friends) {
              return;
            } else if (data.delete) {
              console.log(data.delete);
              this.deleteTweet(data.delete.status.id_str);
            } else if (data.scrub_geo) {
              // location deletion
            } else if (data.limit) {
              // limit notices
            } else if (data.status_withheld) {
              // withheld content notices (status)
            } else if (data.user_withheld) {
              // withheld content notices (user)
            } else if (data.disconnect) {
              // disconnect messages
            } else if (data.warning) {
              // stall warnings
            } else if (data.event) {
              // catch some event
            } else {
              // status
              this.addTweet(data);
              if (this.isMentionsForYou(data)) {
                this.createNotification(`${data.user.name} mentioned to your tweet`, data.text, data.user.profile_image_url_https, 'tweet');
              }
            }
          });
          stream.on('favorite', (data)=>{
            if (data.target.screen_name === this.me.screen_name) {
              this.createNotification(`Your tweet was favorited by ${data.source.name} `, data.target_object.text, data.target.profile_image_url_https, 'favorite');
            }
          });
          stream.on('unfavorite', (data)=>{
            if (data.target.screen_name === this.me.screen_name) {
              this.createNotification(`Your tweet was unfavorited by ${data.source.name} `, data.target_object.text, data.target.profile_image_url_https, 'unfavorite');
            }
          });
          stream.on('follow', (data)=>{
            if (data.target.screen_name === this.me.screen_name) {
              this.createNotification(`${data.source.name} followed you.`, data.source.description, data.source.profile_image_url_https, 'follow');
            }
          });
          stream.on('list_member_added', (data)=>{
            if (data.target.screen_name === this.me.screen_name) {
              this.createNotification(`${data.source.name} added you to the list ${data.target_object.name} `, data.target_object.description, data.source.profile_image_url_https, 'list_member_added');
            }
          });
        });
      },
      selectTweet(tweet) {
        this.selectedTweet = tweet;
      },
      toggleSelectedTweet(tweet) {
        if (this.isSelectedTweet(tweet)){
          this.selectedTweet = {};
        }else{
          this.selectTweet(tweet);
        }
      },
      isSelectedTweet(tweet) {
        return this.selectedTweet && this.selectedTweet.id_str == tweet.id_str;
      },
      favoriteTweet(tweet) {
        console.log(tweet.id_str);
        const favorites_url = tweet.favorited ? 'favorites/destroy' : 'favorites/create';
        const favorites_id = tweet.retweeted_status ? tweet.retweeted_status.id_str : tweet.id_str;
        api.post(favorites_url, {id: favorites_id}, (error, _tweet, response)=>{
          if (error) {
            this.createNotification("Failed to favorite tweet", tweet.text, null, 'fail');
            return console.log('error', error.map((e)=>e.message).join("\n"),  error);
          }
          console.log(_tweet, response);
          if (tweet.favorited){
            this.createNotification("Unfavorited the tweet", tweet.text, null, 'unfavorite');
          }else{
            this.createNotification("Favorite the tweet", tweet.text, null, 'favorite');
          }
          Object.assign(tweet, _tweet);
        });
      },
      retweetTweet(tweet) {
        console.log(tweet.id_str);
        const retweet_url = tweet.retweeted ? 'statuses/destroy' : 'statuses/retweet';
        let retweet_id = tweet.id_str;
        if (tweet.retweeted) {
          api.get('statuses/show', {id: tweet.id_str, include_my_retweet: 1}, (error, _tweet)=>{
            retweet_id = _tweet.current_user_retweet.id_str;
            api.post(retweet_url, {id: retweet_id}, (error, _tweet, response)=>{
              if (error) {
                this.createNotification("Failed to taking back Retweet", tweet.text, null, 'fail');
                console.log('error', error.map((e)=>e.message).join("\n"),  error);
                return;
              }
              console.log(_tweet, response);
              this.createNotification("Took back Retweet", tweet.text, null, 'favorite');
              Object.assign(tweet, _tweet.retweeted_status);
            });
          });
        } else {
          api.post(retweet_url, {id: retweet_id}, (error, _tweet, response)=>{
            if (error) {
              this.createNotification("Failed to Retweet", tweet.text, null, 'fail');
              console.log('error', error.map((e)=>e.message).join("\n"),  error);
              return;
            }
            console.log(_tweet, response);
            this.createNotification("Retweeted the tweet", tweet.text, null, 'favorite');
            Object.assign(tweet, _tweet.retweeted_status);
          });
        }
      },
      showImage(url, size) {
        this.image = {url: url, width: size.w, height: size.h};
        this.showingMediaType = 'photo';
      },
      showVideo(url, mimetype) {
        this.video = {url, mimetype};
        this.showingMediaType = 'video';
      },
      showMedia(media) {
        if(media.type == 'photo'){
          this.showImage(media.media_url, media.sizes.small);
        }else{
          media.video_info.variants.some((v) => {
            if (v.content_type == 'video/mp4') {
              this.showVideo(v.url, v.content_type);
              return true;
            }
          });
        }
      },
      closeMedia() {
        this.image = {};
        this.video = {};
        this.showingMediaType = '';
      },
      sendReply(tweet) {
        console.log(tweet.id_str);
        this.sendReplyDestTweet = tweet;
        this.newTweet.text = `@${tweet.user.screen_name} `;
        this.newTweet.in_reply_to_status_id = tweet.id_str;
        this.$els.submit_box.focus();
      },
      hasMedias(tweet) {
        return 'extended_entities' in tweet && 'media' in tweet.extended_entities;
      },
      isMentionsForYou(tweet) {
        return tweet.text.match(`@${this.me.screen_name}`) && !tweet.retweeted_status;
      },
      body(tweet){
        return tweet.retweeted_status || tweet;
      },
      createTwitterURL(tweet){
        return `https://twitter.com/${tweet.user.screen_name}/status/${tweet.id_str}`;
      },
      jumpToTwitterURL(tweet){
        require('shell').openExternal(this.createTwitterURL(tweet));
      },
      deleteTweet(id_str) {
        this.tweets.forEach((t,i) => {
          if (t.id_str === id_str){
            this.tweets.splice(i, 1);
          }
        });
      },
      createNotification(title, body, icon, kind) {
        const notification = {title, body, kind};
        this.notifications.push(notification);
        const removeMSecond = 6000;
        window.setTimeout(()=>{
          this.removeNotification(notification);
        }, removeMSecond);
        if (!remote.getCurrentWindow().isFocused()) {
          new Notification(title, {icon: icon, body: body});
        }
      },
      removeNotification(notification) {
        this.notifications.forEach((n,i) => {
          if (n === notification){
            this.notifications.splice(i, 1);
          }
        });
      },
      contextMenuOnTweet(tweet) {
        this.selectTweet(tweet);
        contextMenuForTweet.popup(remote.getCurrentWindow());
      },
      contextMenuOnImage() {
        contextMenuForImage.popup(remote.getCurrentWindow());
      },
      saveImage() {
        const basename = path.basename(this.image.url);
        const defaultPath = path.dirname(require.main.filename) + basename;
        dialog.showSaveDialog({defaultPath}, (filename)=>{
          require('request')(this.image.url).pipe(require('fs').createWriteStream(filename)).on('close', ()=>{
            this.createNotification('Saved image', basename, null, 'success');
          });
        });
      },
      detectKeyDown(event) {
        if (event.ctrlKey && event.keyIdentifier == "Enter" && document.activeElement == this.$els.submit_box) {
          this.sendTweet(this.newTweet);
        }
      },
      convertTime(time) {
        const date = new Date(time);
        const diff = Math.floor((this.nowTime.getTime() - date.getTime()) / 1000);
        if (diff < 60) {
          return `${diff}秒`;
        } else if (diff < 3600) {
          return `${Math.floor(diff / 60)}分`;
        } else if (diff < 86400) {
          return `${Math.floor(diff / 3600)}時間`;
        } else if (diff < 31536000) {
          return `${Math.floor(diff / 86400)}日`;
        } else {
          return `${Math.floor(diff / 31536000)}年`;
        }
      }
    }
  });
  if(setting.tokens){
    api = getApi(setting.tokens);
    api.get('account/verify_credentials', {}, (error, data)=>{
      if (error) {
        return console.log('error', error.map((e)=>e.message).join("\n"),  error);
      }
      vm.me = data;
    });
    api.get('statuses/home_timeline', {count: 200}, (error, tweets)=>{
      if (error) {
        console.log('error', error.map((e)=>e.message).join("\n"),  error);
      }
      console.log('tweets',tweets.map((t)=>t.text).join("\n"), tweets);
      tweets.forEach((tweet)=>{
        vm.addTweet(tweet);
      });
    });
    vm.startStreaming();
    window.setInterval(()=>{
      vm.nowTime = new Date();
    }, 1000);
  }else{
    console.warn("did not oauth");
  }
});
